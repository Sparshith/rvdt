...
$DB = new DBPDO();
$adj_id = isset($_GET['adj_id']) && $_GET['adj_id'] ? (int) $_GET['adj_id'] : 0;

$matchups_data = $DB->fetchAll("SELECT round_id, venue_id FROM `matches_judges` WHERE adj_id = ? ORDER BY round_id", array($adj_id));
...
$judge_name =  $judges_mapping[$adj_id];
$_title = 'Matchups for Judge: '. $judge_name;
